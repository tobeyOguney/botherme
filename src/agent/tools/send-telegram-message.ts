...
/**
 * The single chokepoint for outbound text. The PreToolUse hook
 ...
 */
export function buildSendMessageMcpServer(chatId: string, trace: Trace) {
  const sendMessage = tool(
    ...
    async ({ text }) => {
      trace.write({ type: "tool_call", tool: "send_telegram_message", input: { text } });
      try {
        await sendTelegramMessage(chatId, text);
        recordOutbound(chatId);
        trace.write({ type: "tool_result", tool: "send_telegram_message", ok: true });
        return {
          content: [{ type: "text", text: "delivered" }],
        };
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        trace.write({ type: "tool_result", tool: "send_telegram_message", ok: false, result: msg });
        return {
